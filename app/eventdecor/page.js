...
import { CheckCircle, X, ShoppingCart, Filter, Eye, Trash2, Zap, Layout } from "lucide-react";
...
const renderDetails = (service) => {
    // 1. Render Steps (for services like Manicure, Facial, etc.)
    if (Array.isArray(service.steps) && service.steps.length > 0) {
        const stepsArray = service.steps;
        return (
            <div className="mt-6 p-5 border border-gray-200 rounded-2xl bg-white shadow-inner">
                ...
                <ol className="pl-0 space-y-4 text-gray-700">
                    {stepsArray.map((step, index) => {
                        // Logic to handle steps like "Step X: Heading - Detail..."
                        const parts = step.split(/\s*-\s*/, 2); // Split only on the first '-' or '—'
    ...
    // 2. Render Features & Attractive Points (for Event Decor/Design)
    const hasFeatures = Array.isArray(service.features) && service.features.length > 0;
    const hasPoints = Array.isArray(service.attractive_points) && service.attractive_points.length > 0;

    if (hasFeatures || hasPoints) {
        return (
            <div className="mt-6 space-y-6">
                {hasFeatures && (
                    <div className="p-5 border border-gray-200 rounded-2xl bg-white shadow-inner">
                        <h4 className="text-xl font-bold text-gray-800 mb-3 flex items-center gap-2">
                            <Layout className="w-5 h-5 text-indigo-500" /> Design Elements
                        </h4>
                ...
                {hasPoints && (
                    <div className="p-5 border border-gray-200 rounded-2xl bg-white shadow-inner">
                        <h4 className="text-xl font-bold text-gray-800 mb-3 flex items-center gap-2">
                            <Zap className="w-5 h-5 text-yellow-500" /> Attractive Points
                        </h4>
                        <ul className="space-y-2 text-gray-600 text-base list-disc pl-5">
                            {service.attractive_points.map((point, index) => (
    ...
    return null; // No specific details array found
};
    ...
    <div className="mt-[70px] bg-gray-50 min-h-screen">
      ...
      <div className="sticky top-[70px] bg-white z-40 shadow-md border-b">
        <div className="max-w-6xl mx-auto px-4 py-4 flex flex-wrap gap-3 justify-center">
          {["ALL", "Theme", "modern", "PEDICURE", "CLEANSING", "WAXING", "BLEACH", "MAKEUP"].map((cat) => (
            <button
              key={cat}
    ...
    {selectedService && (
        ...
        >
            <motion.div
                ...
                // Increased max-w to 2xl and fixed max-h to use full vertical space
                className="bg-white rounded-3xl shadow-2xl  max-w-3xl relative max-h-[80vh] overflow-y-auto"
            ...
            >
                ...
                <div className="relative w-full h-56">
                    <Image
                        ...
                        // Removed rounded-t-3xl here, adding it to the modal container instead
                ...
                <div className="p-6">
                    ...
                    {/* --- RENDER EVENT DETAILS DYNAMICALLY --- */}
                    {/* This function now handles steps OR features/points based on the data */}
